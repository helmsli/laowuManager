...
function openTaskAgentWin(projectId,taskid,state,projectName,changeStata){
	var from ="myTaskAgent.html?";
		url = projectName+"/projectManager/project_detail_view.html?projectName="+projectName+"&projectId="+projectId+"&state="+state+"&from="+from+"&changeStata="+changeStata;
	if(state=="jzdc")
		{
			//填写尽职调查
			url= projectName+"/myTaskAgent_question.html?projectName="+projectName+"&projectId="+projectId+"&state="+state+"&from="+from;
		}
	location.href=url;
}
//我的代办更多跳转
