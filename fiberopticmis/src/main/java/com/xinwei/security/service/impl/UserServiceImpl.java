...
public class UserServiceImpl  implements UserService {
	...
	@Autowired
	private SyncUserGroupToActivitiService syncUserAndGroupServiceImpl; //同步到activiti中的用户与组服务
	...
	public void save(User user) throws ExistedException {	
		...
		syncUserAndGroupServiceImpl.saveUser(user.getId().toString(), user.getFirstname(), 
			user.getLastname(), user.getPassword(), user.getEmail(),user.getPhone());
	...
	private void saveRole(User user) {
		for (Long roleId : user.getRoleIds()) {
			...
			//将用户放入角色对应的组里
			syncUserAndGroupServiceImpl.saveMembership(user.getId().toString(), roleId.toString());;
		}
	}
	...
	public void updateSelf(User user){
		...
		userDao.update(user);
		//同步activiti中的用户信息
		syncUserAndGroupServiceImpl.updateUser(user.getId().toString(), user.getFirstname(), 
				user.getLastname(), user.getPassword(), user.getEmail(),user.getPhone());
	...
	public void update(User user) {
		...
		//删除activiti中用户组关系
		syncUserAndGroupServiceImpl.deleteMembershipByUId(user.getId().toString());
		//保存用户角色关系
		saveRole(user);
	...
	public void delete(Long id) throws ServiceException {
		...
		//删除activiti中用户组关系
		syncUserAndGroupServiceImpl.deleteMembershipByUId(id.toString());
		userDao.delete(id);
		//删除Activiti中的用户
		syncUserAndGroupServiceImpl.deleteUser(id.toString());
