...
import com.xinwei.fiber.controller.BaseController;
import com.xinwei.security.Constants;
import com.xinwei.security.entity.Company;
import com.xinwei.security.entity.Role;
import com.xinwei.security.entity.User;
import com.xinwei.security.entity.UserRole;
import com.xinwei.security.service.CompanyService;
...
public class UserControllerNew extends BaseController{
	...
	@Value("${roleId_visitor}")
	private Long roleId_visitor;//游客角色ID
	 ...
	 * @return
	 */
	@RequestMapping(value="/getByKeyWords/list", method={RequestMethod.GET, RequestMethod.POST})
	...
	public @ResponseBody String getRigisterUserInfo(Long userId) {	
		...
		if(null != user){
			...
			if(null != company && Constants.Approve.CODE_NOT_APPROVED.toString().equals(company.getCurrentStatus().trim())){
	...
	public @ResponseBody String approval(Long userId,Long companyId,String approvalResult) {	
		...
		if(null != currentUser && null != userId && StringUtil.isNotEmpty(approvalResult)){
		    ...
		    if(null !=user){
		    	//根据审批结果进行处理
		    	if(Constants.Approve.Agree.equals(approvalResult)){
		    		
		    		//如果审核通过，修改用户审核状态为审核通过状态
		    		user.setApprovalStatus(Constants.Approve.CODE_Agree);
		    		//TODO 修改用户角色
		    		...
		    		if(null != company && Constants.Approve.CODE_NOT_APPROVED.toString().equals(company.getCurrentStatus().trim())){
		    			...
		    			//修改单位审核状态为审核通过
		    			company.setCurrentStatus(Constants.Approve.CODE_Agree.toString());
		    			companyServiceImpl.update(company);
		    		}
		    	}else{//审核不通过，
		    		//修改用户审核状态为审核不通过
		    		user.setApprovalStatus(Constants.Approve.CODE_Reject);
		    		//更新用户对象
		    		userService.update(user);
		    		if(null != company){
		    			//删除审核不通过的单位信息
